...
export default function SocialLinks() {
  const socialLinks = [
    ...
    },
    {
      ...
      color: "from-accent-orange to-accent-rust",
      icon: (
        <svg
          className="w-6 h-6"
          fill="none"
  ...
  return (
    <div className="max-w-3xl mx-auto">
      ...
      >
        <span className="mb-1 inline-block -rotate-2 font-hand text-2xl text-accent-orange">
          come along
        </span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {socialLinks.map((link, index) => (
          <motion.a
            ...
            className="group relative card-hover rounded-2xl border-[1.6px] border-line bg-card p-8 shadow-paper transition-all duration-500 hover:border-accent-orange/50 hover:shadow-paper-lg"
            whileHover={{ y: -8, rotate: -0.6 }}
          >
            {/* Gradient background on hover */}
            <div
              className={`absolute inset-0 rounded-2xl bg-gradient-to-br ${link.color} opacity-0 transition-opacity duration-500 group-hover:opacity-[0.08]`}
            />

            <div className="relative z-10">
              <div className="flex items-center gap-4 mb-4">
                <div
                  className={`rounded-xl bg-gradient-to-br ${link.color} p-3 opacity-90 transition-all duration-300 group-hover:opacity-100`}
                >
                  <div className="text-pure-white">{link.icon}</div>
                </div>
                <div>
                  <h3 className="font-serif text-xl font-medium text-ink transition-colors duration-300">
                    {link.name}
                  </h3>
                  <p className="font-hand text-base text-accent-rust">
                    {link.username}
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between">
                ...
                <svg
                  className="h-5 w-5 text-ink-faint transition-colors duration-300 group-hover:text-accent-orange"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
