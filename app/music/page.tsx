...
function TrackCard({ track, index, isTopTrack = false }: { track: SpotifyTrack, index: number, isTopTrack?: boolean }) {
  return (
    <motion.div
      ...
      <div className="relative bg-gradient-to-br from-charcoal-black/90 via-charcoal-black/70 to-void-black/90 backdrop-blur-xl border border-white/5 rounded-2xl p-6 overflow-hidden transition-all duration-700 hover:border-white/10 hover:shadow-2xl hover:shadow-black/50">
        ...
        <div className="relative z-10 flex items-center gap-4">
          ...
          <div className="flex flex-col items-end gap-2 text-xs text-gray-500">
            ...
            {track.playedAt && (
              <span>{formatDistanceToNow(new Date(track.playedAt), { addSuffix: true })}</span>
            )}
            {track.popularity && (
...
export default function MusicPage() {
  ...
  const { data: nowPlaying } = useSWR<NowPlayingTrack>('/api/spotify/now-playing', fetcher, {
    refreshInterval: 30000,
  })
  const { data: recentlyPlayed } = useSWR<{ tracks: SpotifyTrack[] }>('/api/spotify/recently-played', fetcher, {
    refreshInterval: 60000,
  })
  const { data: topTracks } = useSWR<{ tracks: SpotifyTrack[] }>('/api/spotify/top-tracks', fetcher, {
    refreshInterval: 300000, // 5 minutes
  })
  const { data: playlists } = useSWR<{ playlists: SpotifyPlaylist[] }>('/api/spotify/playlists', fetcher, {
        ...
        >
          <div className="max-w-7xl mx-auto px-6 md:px-8">
            <div className="text-center mb-12">
              ...
              >
                a window into my sonic world—what moves me, inspires me, and keeps me going
              </motion.p>
            </div>
                  ...
                  <div className="relative z-10 flex items-center gap-6">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-3 mb-2">
                        <motion.div
                          className={`w-3 h-3 rounded-full ${nowPlaying.isPlaying ? 'bg-green-500' : 'bg-gray-500'}`}
                          animate={nowPlaying.isPlaying ? { opacity: [1, 0.3, 1] } : {}}
            ...
            {selectedTab === 'top' && (
              ...
              >
                {topTracks?.tracks.map((track, index) => (
                  <TrackCard key={track.id} track={track} index={index} isTopTrack={true} />
                ))}
