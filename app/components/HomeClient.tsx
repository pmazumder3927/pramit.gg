...
export default function HomeClient({ posts }: HomeClientProps) {
  const [featuredPosts] = useState<Post[]>(posts.slice(0, 3));
  const [recentPosts] = useState<Post[]>(posts.slice(3));
  const { isLoading, stopLoading } = useLoading(false); // Start with false since we have SSR data
