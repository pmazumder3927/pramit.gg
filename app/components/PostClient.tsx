...
import { Post, generateSlug } from '@/app/lib/supabase'
...
export default function PostClient({ post }: PostClientProps) {
  ...
  useEffect(() => {
    // Check if we need to redirect to slug-based URL
    const slug = generateSlug(post.title)
    if (params.id !== slug && /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(params.id as string)) {
      router.replace(`/post/${slug}`)
    }
  }, [post.title, params.id, router])
