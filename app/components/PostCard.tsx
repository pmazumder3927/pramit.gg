...
export default function PostCard({ post, index, featured = false }: PostCardProps) {
  ...
  const renderMedia = () => {
    ...
    switch (post.type) {
      case 'music':
        return (
          <div className={`relative ${featured ? 'h-32' : 'h-28'} bg-gradient-to-br from-charcoal-black via-void-black to-charcoal-black rounded-xl overflow-hidden mb-6 group-hover:shadow-2xl transition-shadow duration-700`}>
      ...
      case 'climb':
        return (
          <div className={`relative ${featured ? 'aspect-video' : 'aspect-video'} bg-charcoal-black rounded-xl overflow-hidden mb-6 group shadow-xl`}>
  ...
  return (
    <motion.article
      ...
      onClick={handleClick}
    >
      <div className={`relative h-full bg-gradient-to-br from-charcoal-black/90 via-charcoal-black/70 to-void-black/90 backdrop-blur-xl border border-white/5 ${featured ? 'rounded-3xl p-8' : 'rounded-2xl p-6'} overflow-hidden transition-all duration-700 hover:border-white/10 hover:shadow-2xl hover:shadow-black/50`}>
            ...
            <div className="flex items-start justify-between">
              <div className="flex-1">
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <span 
                    className={`text-xs px-2.5 py-1 rounded-full bg-white/5 backdrop-blur-sm border border-white/10 transition-all duration-300 group-hover:bg-white/10 ${featured ? 'font-medium' : 'font-light'}`}
                ...
                <h3 className={`${featured ? 'text-2xl md:text-3xl' : 'text-xl'} font-light leading-tight text-white/90 group-hover:text-white transition-colors duration-300 mb-3`}>
                  {post.title}
                </h3>
                
                {post.content && (
                  <p className={`text-gray-400 ${featured ? 'text-base' : 'text-sm'} leading-relaxed ${featured ? 'line-clamp-3' : 'line-clamp-2'} group-hover:text-gray-300 transition-colors duration-300`}>
                    {getPreview()}
                  </p>
                )}
              </div>
